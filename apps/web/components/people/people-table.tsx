...
> = {
  ...
  churned: { variant: "destructive", className: "opacity-70" },
};
...
const columns: ColumnDef<Person>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2 items-center">
          ...
          <div>
            <p className="text-sm font-medium hover:underline cursor-pointer">{p.name}</p>
  ...
  },
  {
    ...
    header: "Organization",
    cell: ({ row }) => {
      const org = row.original.orgName;
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const p = row.original;
      if (!p.ownerName) return <span className="text-muted-foreground">-</span>;
      return (
        <div className="flex items-center gap-1.5">
          <Avatar className="w-5 h-5">
            <AvatarFallback className="text-[9px]">{p.ownerInitials}</AvatarFallback>
          </Avatar>
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const text = relativeDate(date);
      const diff = date
        ? Math.floor((Date.now() - new Date(date).getTime()) / (1000 * 60 * 60 * 24))
        : Infinity;
      return <span className={cn("text-sm", diff > 14 && "text-muted-foreground")}>{text}</span>;
    },
  },
  {
    accessorKey: "source",
    ...
    cell: () => (
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end">
          ...
          <DropdownMenuItem>Enroll in Sequence</DropdownMenuItem>
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <p className="text-sm text-muted-foreground">
          {table.getFilteredRowModel().rows.length} total people
        </p>
        <Pagination>
          <PaginationContent>
            <PaginationItem>
