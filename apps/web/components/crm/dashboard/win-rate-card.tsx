...
export function WinRateCard({ won, lost, total, rate }: WinRateCardProps) {
  ...
  return (
    <Card className="overflow-hidden rounded-[1.5rem] border-border/70 bg-card/88 shadow-sm backdrop-blur-xl">
      <div className="h-1 bg-gradient-to-r from-emerald-500 via-amber-500 to-rose-500" />
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base font-semibold tracking-tight">Win Rate</CardTitle>
          <Badge
            variant="secondary"
            className={cn("gap-1 rounded-full px-2.5 py-1 text-xs font-medium", trendColor)}
          >
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid grid-cols-3 gap-2">
          <div className="flex flex-col items-center gap-1 rounded-2xl bg-emerald-500/10 px-2 py-3">
            <span className="text-xs text-muted-foreground">Won</span>
            <span className="text-lg font-semibold text-emerald-700 dark:text-emerald-400 tabular-nums">
              {won}
            </span>
          </div>
          <div className="flex flex-col items-center gap-1 rounded-2xl bg-rose-500/10 px-2 py-3">
            <span className="text-xs text-muted-foreground">Lost</span>
            <span className="text-lg font-semibold text-rose-700 dark:text-rose-400 tabular-nums">
              {lost}
            </span>
          </div>
          <div className="flex flex-col items-center gap-1 rounded-2xl bg-muted/60 px-2 py-3">
